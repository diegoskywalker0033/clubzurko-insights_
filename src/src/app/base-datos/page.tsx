...
export default function BaseDatosPage(){
  ...
  const crecByWeek=useMemo(()=>{const b:Record<number,{week:number;citas:number;altas:number}>={};for(const c of cfilt){if(!b[c.week])b[c.week]={week:c.week,citas:0,altas:0};b[c.week].citas+=c.citas;b[c.week].altas+=c.altas}return Object.values(b).sort((a,b2)=>a.week-b2.week)},[cfilt])
  ...
  return(
    <div className="p-6 max-w-[1400px]">
      <PageHeader title="Base de Datos" subtitle="Crecimiento, perfiles y altas semanales"/>
      <div className="bg-white border border-gray-100 rounded-lg px-5 py-4 shadow-card mb-6">
        ...
        <select value={sf} onChange={e=>setSf(e.target.value)} className="text-sm border border-gray-200 rounded-md px-3 py-1.5 bg-white focus:outline-none focus:border-brand-red">
          {['todas','Madrid','Albacete','Barcelona'].map(s=><option key={s} value={s}>{s==='todas'?'Todas las sedes':s}</option>)}</select>
      </div>
      <Div t="Resumen BDD"/>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-2">
        ...
        {['Madrid','Albacete','Barcelona'].map(s=><KpiCard key={s} label={s} value={fmtNum(latest[s]?.total_bdd||0)} sub="voluntarios"/>)}
      </div>
      <Div t="Evolución BDD por mes"/>
      <div className="kpi-card mb-6"><ResponsiveContainer width="100%" height={200}><LineChart data={bddByMes}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" vertical={false}/><XAxis dataKey="mes" tick={{fontSize:10,fill:'#505050'}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:10,fill:'#505050'}} axisLine={false} tickLine={false}/><Tooltip/><Line dataKey="total" name="BDD Activa" stroke="#B11C1F" strokeWidth={2.5} dot={{fill:'#B11C1F',r:4}}/></LineChart></ResponsiveContainer></div>
      <Div t="Altas semanales"/>
      <div className="kpi-card mb-6"><ResponsiveContainer width="100%" height={200}><BarChart data={crecByWeek} barGap={2} barCategoryGap="30%"><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" vertical={false}/><XAxis dataKey="week" tickFormatter={v=>`W${v}`} tick={{fontSize:10,fill:'#505050'}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:10,fill:'#505050'}} axisLine={false} tickLine={false}/><Tooltip/><Legend wrapperStyle={{fontSize:11}}/><Bar dataKey="citas" name="Citas" fill="#C6C6C6" radius={[2,2,0,0]}/><Bar dataKey="altas" name="Altas" fill="#B11C1F" radius={[2,2,0,0]}/></BarChart></ResponsiveContainer></div>
