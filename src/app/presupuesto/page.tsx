...
export default function PresupuestoPage() {
  ...
  // Flat list of all real partidas (not subtotals) for chart
  ...
  const chartData = todasPartidas.map(p => ({
    name: p.categoria.length > 25 ? p.categoria.substring(0,25)+'…' : p.categoria,
    fullName: p.categoria,
    Presupuesto: p.presupuesto_total,
    Gastado: p.gastado,
    over: p.restante < 0,
  }))

  const pendientes = todasPartidas.filter(p => p.gastado === 0)
  const conGasto = todasPartidas.filter(p => p.gastado > 0)
  const totalPendientePresup = pendientes.reduce((s,p)=>s+p.presupuesto_total,0)

  return (
    <div className="p-6 max-w-[1400px]">
      <PageHeader title="Presupuesto 2026" subtitle="Gasto real vs presupuesto · Fuente: Gasto real 2026"/>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-2">
        ...
        <KpiCard label="% Ejecución" value={`${(data.total_gastado/data.total_presupuesto*100).toFixed(1)}%`}
          sub={`${fmtEur(data.total_gastado)} de ${fmtEur(data.total_presupuesto)}`}/>
      </div>
      ...
      {data.grupos.map((grupo, gi) => (
        <div key={gi}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
            ...
            <KpiCard label="% Ejecución"
              value={grupo.presupuesto_total > 0 ? `${(grupo.gastado/grupo.presupuesto_total*100).toFixed(1)}%` : grupo.gastado > 0 ? '∞' : '0%'}/>
          </div>
                    ...
                    {grupo.partidas.map((p, pi) => {
                      ...
                      return (
                        <tr key={pi}>
                          ...
                          <td className={over?'text-red-600 font-medium':''}>{fmtEur(p.restante)}</td>
                          <td>
                          ...
                          <td>
                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                              p.gastado === 0 ? 'bg-gray-100 text-gray-600' :
                              over ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                            }`}>
                              {p.gastado === 0 ? 'Pendiente' : over ? 'Desviado' : 'En presupuesto'}
                            </span>
                          </td>
                        </tr>
                      )
      ...
      <div className="kpi-card bg-gray-50 border-2 border-gray-200 mt-4 mb-6">
        <div className="flex flex-wrap justify-between items-center gap-4">
          <span className="text-sm font-bold text-zurko-black" style={{fontFamily:'Sansation,sans-serif'}}>TOTAL PRESUPUESTO 2026</span>
          <div className="flex gap-6 text-sm">
            ...
            <div><span className="text-zurko-dark text-xs">Disponible: </span><span className="font-bold">{fmtEur(data.total_restante)}</span></div>
          </div>
        </div>
      </div>

      {/* Gráfico partidas con gasto — barras verticales con presupuesto y gastado */}
      ...
      <div className="kpi-card mb-6">
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={chartData.filter(d=>d.Presupuesto > 0 || d.Gastado > 0)} barGap={2} barCategoryGap="25%">
            ...
            <XAxis dataKey="name" tick={{fontSize:9,fill:'#505050'}} axisLine={false} tickLine={false} interval={0} angle={-15} textAnchor="end" height={50}/>
      ...
      {/* Pendientes con total */}
      {pendientes.length > 0 && (
        <>
          <Divider title="Partidas pendientes de ejecutar"/>
          <div className="kpi-card p-0 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full data-table">
                <thead>
                  <tr><th>Partida</th><th>Presupuesto asignado</th><th>Estado</th></tr>
                </thead>
                <tbody>
                  {pendientes.map((p,i)=>(
                    <tr key={i}>
                      <td className="font-medium text-xs">{p.categoria}</td>
                      <td>{fmtEur(p.presupuesto_total)}</td>
                      <td><span className="text-[10px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">Sin gasto</span></td>
                    </tr>
                  ))}
                  <tr className="bg-gray-50 font-bold">
                    <td>TOTAL PENDIENTE</td>
                    <td className="text-brand-red">{fmtEur(totalPendientePresup)}</td>
                    <td></td>
                  </tr>
                </tbody>
              </table>
