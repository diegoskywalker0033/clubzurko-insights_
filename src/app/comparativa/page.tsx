...
function DeltaBadge({v2026, v2025}:{v2026:number; v2025:number}) {
  ...
  const pos = pct >= 0
  return (
    <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full ml-2 ${pos?'bg-green-100 text-green-700':'bg-red-100 text-red-700'}`}>
...
export default function ComparativaPage() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const d26 = useMemo(() => sedeFiltro==='todas' ? data2026 : data2026.filter((r:any)=>r.sede===sedeFiltro), [data2026, sedeFiltro])
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const d25 = useMemo(() => sedeFiltro==='todas' ? data2025 : data2025.filter((r:any)=>r.sede===sedeFiltro), [data2025, sedeFiltro])
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const ag25 = useMemo(() => sedeFiltro==='todas' ? agenda2025 : agenda2025.filter((r:any)=>r.sede===sedeFiltro), [agenda2025, sedeFiltro])
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const cr26 = useMemo(() => sedeFiltro==='todas' ? crec2026 : crec2026.filter((r:any)=>r.sede===sedeFiltro), [crec2026, sedeFiltro])
  ...
  const weeklyChart = useMemo(() => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const agg = (data:any[], key:string) => { const m=new Map<number,number>(); data.forEach((r:any)=>{const w=r.week||r.S; m.set(w,(m.get(w)||0)+(Number(r[key])||0))}); return m }
    const r26=agg(d26,'reclutados'), i26=agg(d26,'iniciados'), r25=agg(d25,'reclutados'), i25=agg(d25,'iniciados')
  ...
  const altasChart = useMemo(() => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const agg = (data:any[]) => { const m=new Map<number,number>(); data.forEach((r:any)=>m.set(r.week,(m.get(r.week)||0)+(Number(r.altas)||0))); return m }
  ...
  return (
    <div className="p-6 max-w-[1400px]">
      <PageHeader title="Comparativa 2025 vs 2026" subtitle="Same Period Last Year · Reclutamiento y Altas BDD"/>
      ...
      <Divider title="KPIs principales"/>
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="kpi-card border-t-4 border-t-brand-red">
          ...
          <div className="space-y-2 text-sm">
            {[['Reclutados',reclu26],['Inicios',ini26],['Solicitados',sol26],['N proyectos',proy26],['Proyectos en N',pn26]].map(([l,v])=>(
              <div key={String(l)} className="flex justify-between"><span className="text-zurko-dark">{l}</span><span className="font-bold">{fmtNum(Number(v))}</span></div>
            ))}
        ...
        <div className="kpi-card border-t-4 border-t-zurko-dark">
          ...
          <div className="space-y-2 text-sm">
            {[['Reclutados',reclu25],['Inicios',ini25],['Solicitados',sol25],['N proyectos',proy25]].map(([l,v])=>(
              <div key={String(l)} className="flex justify-between"><span className="text-zurko-dark">{l}</span><span className="font-bold">{fmtNum(Number(v))}</span></div>
            ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
        {[['Δ Reclutados',reclu26-reclu25,reclu25],['Δ Inicios',ini26-ini25,ini25],['Δ Altas BDD',altas26-altas25,altas25]].map(([l,d,base])=>(
          <div key={String(l)} className="kpi-card">
            <p className="kpi-label mb-1">{l}</p>
            <p className={`text-2xl font-bold ${Number(d)>=0?'text-green-600':'text-red-600'}`} style={{fontFamily:'Sansation,sans-serif'}}>
              {Number(d)>=0?'+':''}{fmtNum(Number(d))}
            </p>
            <DeltaBadge v2026={Number(d)+Number(base)} v2025={Number(base)}/>
          </div>
        ))}
      </div>
      ...
      <Divider title="Altas BDD semanales 2025 vs 2026"/>
      <div className="grid grid-cols-2 gap-3 mb-4">
        <KpiCard label="Altas BDD 2026" value={fmtNum(altas26)} accent/>
